...
function calculateFactorial(aNum) {
    var aFactorial = 1;
    while (aNum > 1) {
        aFactorial *= aNum;
        aNum--;
...
function removeNegativeNumbers(negative) {
...
var actArray = [2, 3, 5, 1];
var getSumArray = sumArray(actArray);
...
function tempC2F(tempC = []) {
    var index = 0;
    var tempInF = [];
    while (index < tempC.length) {
        var tempF = (tempC[index] * 9 / 5) + 32;
        tempInF.push(tempF);
        index++;
    }
    return tempInF;
}
var tInF = [20, 37, 25, 19];
var listInF = tempC2F(tInF);
